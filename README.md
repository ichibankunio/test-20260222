...
- コインと美男子ポートレート欠片ピクセル描画は `kage` シェーダー + 三角形バッチで実行
...
- 操作:
  ...
  - `F3`: パフォーマンス計測HUDの表示切り替え（平均/最大ms、弾数、粒子数を表示）
  - 右上の `DBG:通常` / `DBG:無敵` ボタンでデバッグ用の無敵状態を切り替え（無敵中に接触した弾は消滅し、消滅エフェクトが出る）
  - ゲームオーバー中にタップ / クリック / `SPACE` / `↑` / `W`: 現在ステージをリトライ
  - `ESC`: 終了

## Performance Investigation

- `F3` を押すと、フレーム内の主要区間を分解した計測HUDを表示できます。
- `target 16.67ms` は 60fps の1フレーム予算です。`upd` または `drw` の `avg/max` がこれを超える区間をボトルネック候補として確認してください。
- `drw` 行の `bg` (背景) / `dan` (弾描画) と、`blt` (弾数), `ptc` (パーティクル数), `shd` (ポートレート破片数) の相関を見ると、負荷増加の原因を切り分けやすくなります。
