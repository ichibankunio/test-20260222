...
- 操作:
  ...
  - `F3`: パフォーマンス計測ログの出力切り替え（平均/最大ms、弾数、粒子数、ボトルネック分析をログ出力）
...
- `F3` を押すと、約2秒ごとに `[PERF]` ログが出力されます。
- ログには `target 16.67ms` に対する `avg/max`、オブジェクト数、ボトルネック区間、簡易分析 (`analysis`/`hint`) が含まれます。
- `analysis` が `budget exceeded` の場合は 60fps 予算超過、`hint` で指摘された区間から優先的に改善してください。
